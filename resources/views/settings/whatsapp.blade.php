    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="d-flex justify-content-end mb-2">
                <a href="javascript:void(0);" class="btn btn-sm btn-primary" id="new-whatsapp-template-form-btn" data-toggle="modal"
                   data-target="#whatsappTemplateModal">Add New
                    Template</a>
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Whatsapp Template</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{route('settings-add-wa-template')}}" id="whatsapp-template-form" method="POST">
                    @csrf
                    <input type="hidden" name="id" value="0">
                    <div class="modal-body">
                        <input type="hidden" name="action" value="manage-whatsapp-template"/>
...
@section('js')
    <script>
        $(function () {
            $(document).on('click', '#new-whatsapp-template-form-btn', () => {
                $('input[name="id"]').val(0);
                $('#whatsapp-template-form').trigger("reset");
            });
            $(document).on('click', '.editTemplate', function () {
                let id = $(this).attr('data-id');
                $.get(`{{route('settings-get-wa-template')}}/${id}`, (res) => {
                    if (res.error === 0) {
            ...
            $(document).on('click', '.removeTemplate', function () {
                ...
                if (confirmR) {
                    $.post(`{{route('settings-delete-wa-template')}}`, {id: id, _token: '{{csrf_token()}}'}, (res) => {
                        if (res.error === 0) {
